...
- **Pick a scratch register that isn't the data register.** The standard Pattern E template uses `v0` as scratch for the `fps_switch` / `frame_phase` reads. If the original `addiu/sh` pair uses `v0` as its data register, scratch will clobber the value before it's stored. **Use `at` (`$1`) as scratch in that case** — same template, just rename `v0` → `at`. Compiled OoT actors do pick `v0` as the field-decrement register (seen in En_Ta blinkTimer and En_Ossan blinkTimer in B62), so this isn't theoretical. The fix is mechanical: when generating hooks, check whether the data register collides with the scratch, and substitute `at` if so.

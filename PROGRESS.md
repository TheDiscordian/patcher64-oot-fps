...
| **Player Hover Boots animation** (user-reported during PR #7 testing) | `hoverBootsTimer` (u8 at Player offset 0x883) seeds to 19 on takeoff (`z_player.c:11048`), decrements once per Update at `z_player.c:11033`, drives the alpha-fade-in / pulse formula at `z_player.c:12218-12230` (`hoverBootsTimer >= 15` 4-frame fade-in window, `hoverBootsTimer < 15` cosine pulse, `hoverBootsTimer >= 9` cap on `sp5C`). Intermediate threshold checks → **tick-mod required** per FIX_PATTERNS. Candidate hook site: `sb t7, 0x883(a1)` at `Player_UpdateHoverBoots+0x2C` = RAM `0x80845BB4` (Player overlay loads at `0x808301C0`, decomp `.o` offset `0x159F4`). User symptom (verbatim): "The effect isn't visible right away, it should be" — the bug is at the **start** of the animation, not the end. The alpha-fade-in window (first 4 frames where `hoverBootsTimer >= 15`) compresses from 0.2 s @ 20 fps to 0.13 s @ 30 fps, so the circle never ramps to a perceptible alpha before the formula drops into the cosine-pulse branch; the effect reads as not appearing right away. The full 19-frame animation also expires in 0.63 s instead of 0.95 s overall. Single-site tick-mod hook. |
| **En_Bw (Torch Slug) stuck-in-air after stun→re-ignite** (user-reported during PR #7 testing) | After being frozen by ice arrow (`damageReaction == 0xE` sets `iceTimer = 32` at `z_en_bw.c:662`) and recovering when `actor.colorFilterTimer == 0` at `z_en_bw.c:670` (state func `func_809D0424`), the slug returns fire (state transition to `func_809CE9A8`) but gets stuck floating in the air. `actor.gravity = -2.0f` is set once in init (`z_en_bw.c:152`) and not re-applied. Candidate causes — needs deeper trace before writing a hook: (1) `actor.velocity.y` not being zeroed on stun-exit so a stale upward velocity from before-stun keeps the slug aloft; (2) gravity is zeroed at some point in the stun state and not restored; (3) `iceTimer` (32-frame seed → tick-mod candidate) misaligning the recovery transition timing. Open question: which exact `func_809D...` state-handler runs after recovery and what is its first interaction with `velocity.y` / `gravity` / `bgCheckFlags`. |
